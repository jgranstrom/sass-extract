...
function verifyImportedFunctions(rendered, sourceFile, functionsFile) {
  ...
  expect(rendered.vars.global.$functionVariable.declarations[0].expression).to.equal(`'function-variable' !global`);
  ...
  expect(rendered.vars.global.$functionVariable.declarations[1].expression).to.equal(`'should-not-happen' !global`);

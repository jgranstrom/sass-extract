...
export function injectExtractionFunctions(fileId, declarations, dependentDeclarations, { globalDeclarationResultHandler }, sass) {
  ...
  declarations.implicitGlobals.forEach((declaration, idx) => {
    const { fnName, injectedFunction, injectedCode } = createInjection(fileId, FN_PREFIX_IMPLICIT_GLOBAL, declaration, idx, globalDeclarationResultHandler, sass);
  ...
  declarations.explicitGlobals.forEach((declaration, idx) => {
    const { fnName, injectedFunction, injectedCode } = createInjection(fileId, FN_PREFIX_EXPLICIT_GLOBAL, declaration, idx, globalDeclarationResultHandler, sass);
    injectedFunctions[fnName] = injectedFunction;
    injectedData += injectedCode;
  });
  ...
  dependentDeclarations.forEach(({ declaration, decFileId }, idx) => {
    ...
    const { fnName, injectedFunction, injectedCode } = createInjection(fileId, FN_PREFIX_DEPENDENT_GLOBAL, declaration, idx, globalDeclarationResultHandler, sass);
    injectedFunctions[fnName] = injectedFunction;
    injectedData += injectedCode;
  });  
